...
/**
 * Start background tracking (Android uses startService with interval, iOS uses startTracking)
 */
export const startNativeTracking = async (intervalSec = 5) => {
  ...
  try {
    ...
    if (Platform.OS === "android") {
      if (typeof LocationServiceBridge.startService === "function") {
        await LocationServiceBridge.startService(intervalSec); // wait for promise
        console.log("Called android startService (awaited)");
        return true;
      } else {
